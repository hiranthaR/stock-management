...
public class DashboardController implements Initializable {
    ...
    @FXML
    private HBox menuMenu;

    public void initialize(URL url, ResourceBundle resourceBundle) {

        stackPane = panesContainer;

        txtTitle.setText(CurrentAdmin.getInstance().getCurrentAdmin().getName() + " @ Dehannegama Hardware");
        ...
        if (!Permissions.checkPermission(CurrentAdmin.getInstance().getCurrentAdmin().getLevel(), Permissions.ADD_CUSTOMER)) {
            menuCustomers.setVisible(false);
            menuCustomers.setManaged(false);
        }
    ...
    private void changePane(AnchorPane pane) {
        if (!panesContainer.getChildren().contains(pane)) {
            ...
            animation.getTimeline().setOnFinished(e -> {
                if (panesContainer.getChildren().size() > 1) {
                    panesContainer.getChildren().remove(currentPane);
                }
                currentPane = pane;
            });
            animation.play();
