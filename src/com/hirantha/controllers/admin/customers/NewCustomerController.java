...
public class NewCustomerController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        btnSave.setOnMouseClicked(e -> {
            if (isCorrect()) {
                if (goingToUpdate) {
                    CustomerQueries.getInstance().updateCustomer(createCustomer(), true);
                    goingToUpdate = false;
                } else {
                    CustomerQueries.getInstance().insertCustomer(createCustomer(), true);
                }
                try {
...
////            DashboardController.stackPane.getChildren().remove(btnCancel.getParent());
//        });
        try {
            customerController.readRows();
        } catch (IOException e) {
