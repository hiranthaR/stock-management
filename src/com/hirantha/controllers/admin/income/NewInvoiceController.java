...
//settings if radio buttons
        ...
        btnPrint.setOnMouseClicked(e -> printReport(Integer.parseInt(invoice.get_id())));

        if (invoice == null) btnPrint.setVisible(false);

        btnPrintAndSave.setOnMouseClicked(e -> {
            if (save())
                printReport(Integer.parseInt(invoice.get_id()));
        });
    }
    ...
    private boolean save() {
        if (checkInvoiceItemProperties()) {
            if (goingToUpdate) {
                ...
                invoice = InvoiceQueries.getInstance().insertInvoice(createInvoice());
            }
            try {
    ...
    private void printReport(int id) {
        String path = "./reports/income.jrxml";
        try {
        ...
        if (checkTableItemProperties()) {
            ...
            for (int i = 0; i < table.getItems().size(); i++) {
                ...
                if (selectedItem.getItemCode().equals(tableItem.getItemId()) && tableItem.getCostPerItem() == Double.parseDouble(txtCostPerItem.getText())) {
                    ...
                    txtBillCost.setText(String.valueOf(Double.parseDouble(txtBillCost.getText()) + invoiceTableItem.getCostPerItem() * Integer.parseInt(txtQuanitiy.getText())));
                    clearTableData();
                    return;
                }
            }
            invoiceTableItem = createTableItem();
            table.getItems().add(invoiceTableItem);
            clearTableData();
